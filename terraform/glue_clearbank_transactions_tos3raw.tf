resource "aws_s3_object" "datalake_cb_transactions_tos3raw" {
...
resource "aws_s3_object" "datalake_cb_transactions_tos3raw_data_catalog" {
  bucket = local.glue_assets_bucket_name
  key    = "${local.project_name}/scripts/clearbank_transactions_to_s3_raw/data_catalog.py"
  source = "../src/lambdas/clearbank_transactions_to_s3_raw/data_catalog.py"

  etag = filemd5("../src/lambdas/clearbank_transactions_to_s3_raw/data_catalog.py")
}


resource "aws_glue_job" "datalake_cb_transactions_tos3raw" {
  name        = "${local.prefix}-clearbank-transactions-tos3raw"
  role_arn    = aws_iam_role.iam_for_clearbank_glue_etl.arn
  max_retries = "0"
  timeout     = 120
  command {
    name            = "pythonshell"
    script_location = "s3://${local.glue_assets_bucket_name}/${aws_s3_object.datalake_cb_transactions_tos3raw.key}"
    python_version  = 3.9
  }
  execution_property {
  ...
  default_arguments = {
   ...
   "--extra-py-files" = join(",", [
      ...
      "s3://${local.glue_assets_bucket_name}/${aws_s3_object.glue_custom_functions.key}",
       "s3://${local.glue_assets_bucket_name}/${aws_s3_object.datalake_cb_transactions_tos3raw_data_catalog.key}",
    ])
